 ...
 */
export const generateIndex = generatorFrom(async function* (
  mac: ManfredAwesomicCV
) {
  const styles = await fs.readFile(
    path.resolve(__dirname, "../../generated/styles.css"),
    "utf8"
  );

  const fullName =
    mac.aboutMe.profile.name + " " + mac.aboutMe.profile.surnames;
  ...
  yield `
    ...
    <html>
        ...
        <div class="left-column-holder">
            <div class="left-column">${generateLeftColumn(mac)}</div>
        </div>
        <div class="right-column-holder">
            <div class="right-column">${generateRightColumn(mac)}</div>
        </div>
        </body>
    </html>
  `;
